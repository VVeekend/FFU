...
NGame = {
	...
	LAG_DAYS_FOR_PAUSE = 280,						-- Days of client lag for pause of gamespeed.
	...
	HANDS_OFF_START_TAG = "SWE",					-- tag for player country for -hands_off runs. use an existing tag that is less likely to affect the game
},

NDiplomacy = {
	DIPLOMACY_REQUEST_EXPIRY_DAYS = 10,
	...
	BASE_BOOST_PARTY_POPULARITY_DAILY_PP = 0.0, 	-- Daily pp cost for boost party popularity
	BASE_BOOST_PARTY_POPULARITY_DAILY_DRIFT = 0.0, 	-- Daily amount of popularity that will be added by the activity.
	BASE_STAGE_COUP_DAILY_PP = 1000,					-- Daily pp cost for staging a coup
	...
	VOLUNTEERS_PER_TARGET_PROVINCE = 0.50,			-- Each province owned by the target country contributes this amount of volunteers to the limit.
	VOLUNTEERS_PER_COUNTRY_ARMY = 1.50,				-- Each army unit owned by the source country contributes this amount of volunteers to the limit.
	VOLUNTEERS_RETURN_EQUIPMENT = 0.95,				-- Returning volunteers keep this much equipment
	VOLUNTEERS_TRANSFER_SPEED = 0,					-- days to transfer a unit to another nation
	...
	TENSION_TIME_SCALE_START_DATE = "1931.1.1.12",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	...
	GUARANTEE_COST = 1000,							-- Scale with the number of already guaranteed countries.
	...
	FRONT_IS_DANGEROUS = -20,						-- AI should be unwilling to enter accept a call to war if front is too dangerous.
	...
	JOIN_FACTION_LIMIT_CHANGE_AT_WAR = 0.2,			-- if in defensive war this or your modifier is counted as limit to join factions (so if you have 80% join limit this now means you can join at 50%)
...
NCountry = {
	...
	COUNTRY_SCORE_MULTIPLIER = 0,					-- Weight of the country score.
	ARMY_SCORE_MULTIPLIER = 0,					-- Based on number of armies.
	NAVY_SCORE_MULTIPLIER = 0,					-- Based on number of navies.
	AIR_SCORE_MULTIPLIER = 0,						-- Based on number of planes (which is typically a lot).
	...
	MAX_PROPAGANDA_STABILITY_IMPACT = -0.0,			-- Max total penalty from operative performing the propaganda mission in a country
	MAX_PROPAGANDA_WAR_SUPPORT_IMPACT = -0.0,		-- Max total penalty from operative performing the propaganda mission in a country
	PROPAGANDA_STABILITY_DAILY_DECAY = 0.000,		-- Amount of stability recovered daily from propaganda effort
	PROPAGANDA_WAR_SUPPORT_DAILY_DECAY = 0.000,		-- Amount of war support recovered daily from war support effort
	...
	SPECIAL_FORCES_CAP_MIN = 1000,					-- You can have a minimum of this many special forces battalions, regardless of the number of non-special forces battalions you have, this can also be modified by a country modifier
...
NResistance = {
	...
	COMPLIANCE_GROWTH_BASE = 1.000, -- base compliance grow
	COMPLIANCE_GROWTH_MIN = -100.0, -- min compliance grow
	COMPLIANCE_GROWTH_MAX = 100.0, -- max compliance grow
	
	COMPLIANCE_GROWTH_IS_AT_PEACE = 30, -- compliance growth buff at peace
...
NProduction = {
	...
	MIN_POSSIBLE_TRAINING_MANPOWER = 1000000,	-- How many deployment lines minimum can be training
	...
	EQUIPMENT_MODULE_ADD_XP_COST = 0.0,					-- XP cost for adding a new equipment module in an empty slot when creating an equipment variant.
...
NMilitary = {
	...
	CORPS_COMMANDER_DIVISIONS_CAP = 96,			-- how many divisions a corps commander is limited to. 0 = inf, < 0 = blocked
	...
	BASE_FORT_PENALTY = -0.10, 					   -- fort penalty
	...
	UNIT_EXPERIENCE_PER_TRAINING_DAY = 0.0,
	...
	TRAINING_ATTRITION = 0.0,		  			   -- amount of extra attrition from being in training
...
NAir = {
	...
	AIR_REGION_SUPERIORITY_PIXEL_SCALE = 0.03,           -- air superiority scale = superiority/(pixels*this)
	...
	AIR_WING_XP_TRAINING_MISSION_GAIN_DAILY = 4.5, 						--Daily gain when running training exercise mission
...
NNavy = {
	...
	NAVAL_RANGE_TO_INGAME_DISTANCE = 0.20,							-- Scale the ship stats "naval_range" to the ingame distance
	...
	HOLD_MISSION_MOVEMENT_COST = 0.5,								-- ships on hold cost this much fuel while moving
	...
	TRAINING_ACCIDENT_CHANCES = 0.0,						-- Chances one ship get damage each hour while on training 
	...
	SCREENING_EFFICIENCY_PENALTY_ON_MINIMUM_POSITIONING				= 0.8,  -- screening efficiency (screen to capital ratio) at 0% positioning
...
NAI = {
	...
	DIPLOMACY_ACCEPT_VOLUNTEERS_BASE = 10,		-- Base value of volunteer acceptance (help is welcome)
	DIPLOMACY_ACCEPT_ATTACHE_BASE = 0,			-- Base value of attache acceptance (help is welcome)
...
NOperatives = {
	...
	BOOST_IDEOLOGY_NATIONAL_COVERAGE_FACTOR = 0.0,				-- used to compute the drift factor as follow: BASE * SUB_NETWORK_NC * BOOST_IDEOLOGY_DEFENSE_FACTOR
	BOOST_IDEOLOGY_MAX_DRIFT_BY_OPERATIVE = 0.0,				-- the maximum drift an operative can cause, a negative value means no maximum
	BOOST_IDEOLOGY_DRIFT_STACKING_FACTOR = 0.0,				-- multiplied to the drift of an operative for each operative after the first one, with the greatest drift. So if we have the following drift values [ 0.1, 0.3, 0.2 ], the factor is applied twice for the lowest value and once for the 2nd lowest one as such : [ 0.3, 0.2 * D, 0.1 * D * D ] and then the result is summed up to give the final drift value.
	BOOST_IDEOLOGY_DEFENSE_FACTOR = 0.0,					-- multiplied to the target's defense to get the amount of drift to remove from each operative's drift
	BOOST_IDEOLOGY_DAILY_XP_GAIN = 0.0,
	...
	OPERATIVE_BASE_PROPAGANDA_POWER = 0.0000,					-- Base amount of daily war support and stability change when an operative is assigned to propaganda
	PROPAGANDA_SUB_NETWORK_STRENGTH_FACTOR = 0.0,				-- Multiplied to the network strength before being multiplied to the Stability/WarSupport drift caused by an operative 
	...
	PROPAGANDA_OPERATIVE_STACKING_FACTOR = 0.0,				-- Multiplied to the Stability/WarSupport drift values of each operative after the one with the greatest values. The process is done separatly for Stability and WarSupport
	PROPAGANDA_COUNTRY_STACKING_FACTOR = 0.0,				-- Multiplied to the Stability/WarSupport drift values of each country after the one with the greatest values. The process is done separatly for Stability and WarSupport
	PROPAGANDA_DAILY_XP_GAIN = 0.0,
	...
	MIN_NATIONAL_COVERAGE_FOR_BOOST_IDEOLOGY = 1.00,			-- Minimum network coverage required to start the mission (the code ensures that a network exists at all)
	MIN_NATIONAL_COVERAGE_FOR_PROPAGANDA = 1.00,			-- Minimum network coverage required to start the mission (the code ensures that a network exists at all)
